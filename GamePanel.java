...
public class GamePanel extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
	...
	public GamePanel(boolean levelSelect, boolean edit, boolean play, String levelName, long ms)
		...
		// enemies
		tabGoblin = new Goblin(this, knight, TAB_X - 110, 20, Goblin.width, Goblin.height, goblinRunLeft,
				goblinRunRight, false);
		tabTurret = new Turret(this, TAB_X - 110, 100, Turret.width, Turret.height, turretRight, turLeft, turRight,
				false);
	...
	public void draw(Graphics2D g) throws IOException {
		...
		if (instructions) {
			...
			g.drawString(
					"Press ESC to return to main menu / level select at ANY point. ** Remember to save your levels!", 5,
					545);
		...
		// in the main menu
			...
			g.drawString(
					"Press ESC to return to main menu / level select at ANY point in the game. ** Remember to save your levels!",
					5, 545);
			...
			if (knight.invincibleStart != -1 && System.currentTimeMillis() - knight.invincibleStart >= knight.iFrames)
				knight.invincibleStart = -1;
	...
	// calls the move metahod of all other methods
	public void move() {
		if (gameEnd)
			return;
		knight.move();
	...
	public void checkCollision() {
		...
		if (edit) {
			...
			for (Block b : elements) {
				...
				//checks if the knight has just collided, if so it should still be blocked
				if (!checkVertical && knight.x == b.x - knight.width - 1
						&& ((knight.y >= b.y && knight.y <= b.y + b.height)
								|| (knight.y + knight.height > b.y && knight.y + knight.height <= b.y + b.height)
								|| knight.y <= b.y && knight.y + knight.height >= b.y + b.height)) {
					if (Player.isCentered)
				...
				//checks if the knight has just collided, if so it should still be blocked
				if (!checkVertical && knight.x == b.x + b.width + 1
						&& ((knight.y >= b.y && knight.y <= b.y + b.height)
								|| (knight.y + knight.height > b.y && knight.y + knight.height <= b.y + b.height)
								|| knight.y <= b.y && knight.y + knight.height >= b.y + b.height)) {
					if (Player.isCentered)
		...
		for (Block b : elements) {
			if (getClass(b).equals("CrackedStone") && ((CrackedStone) b).startBreak != -1
					&& System.currentTimeMillis() - ((CrackedStone) b).startBreak >= CrackedStone.breakTime) {
				remove.add(b);
	...
	public void keyPressed(KeyEvent e) {
		...
		if (instructions) {
			...
			if (e.getKeyCode() == 27) {
				running = false;
			} else if (e.getKeyCode() == 10 && indicatorPos == 320) {
				try {
					play = false;
					levelSelect = false;
					edit = true;
			...
			if (e.getKeyCode() == 37) {
				...
				// save the file
				if (!saveLevel()) {
					JOptionPane.showMessageDialog(this, "Remember to have exactly one portal and one chest.",
			...
			if (sidebarPressed) {
				...
				if (tabPressed.equals("blocks")) {
					...
					if (curDragging.equals(tabGoblin)) {
						try {
							elements.add(new Goblin(this, knight, TAB_X - 110, 20, Goblin.width, Goblin.height,
									goblinRunLeft, goblinRunRight, false));
					...
					if (curDragging.equals(tabTurret)) {
						try {
							elements.add(new Turret(this, TAB_X - 110, 100, Turret.width, Turret.height, turretRight,
					...
					if (curDragging.equals(tabTurretLeft)) {
						try {
							elements.add(new Turret(this, TAB_X - 110, 100, Turret.width, Turret.height, turretLeft,
									true, false, true));
						} catch (IOException IOE) {
						}
						curDragging = elements.get(elements.size() - 1);
					...
					if (line.startsWith(title)) {
						...
						for (int i = 1; i < words.length; i++) {
							...
							if (inputs[0].equals("Ice")) {
								...
								if (play) {
									elements.add(new Goblin(this, knight, Integer.parseInt(inputs[1]),
								...
								if (inputs[5].equals("true")) {
									elements.add(new Turret(this, Integer.parseInt(inputs[1]),
											Integer.parseInt(inputs[2]), Integer.parseInt(inputs[3]),
											Integer.parseInt(inputs[4]), turretLeft, true, false, true));

								} else {
									elements.add(new Turret(this, Integer.parseInt(inputs[1]),
											Integer.parseInt(inputs[2]), Integer.parseInt(inputs[3]),
											Integer.parseInt(inputs[4]), turretRight, turLeft, turRight, true));
	...
	public Block decipherBlock(Block b, int x, int y, int width, int height) throws IOException {
		...
		if (className.equals("Stone")) {
			...
			return new Goblin(this, knight, x, y, width, height, goblinRunLeft, goblinRunRight, false);
			// }
	...
	public boolean saveLevel() {
		...
		if (!levelSaved && prevSavedTitle.isEmpty()) { // if user is creating a fresh new level
			...
			int option = JOptionPane.showOptionDialog(this, textField, "Name your level!", JOptionPane.OK_CANCEL_OPTION,
					JOptionPane.INFORMATION_MESSAGE, null, null, null);
	...
	// checks if a level can be played
	public boolean checkValid() {
		boolean hasChest = false;
		boolean hasPortal = false;

		// loops through the elements for a chest and portal
		for (Block b : elements) {
